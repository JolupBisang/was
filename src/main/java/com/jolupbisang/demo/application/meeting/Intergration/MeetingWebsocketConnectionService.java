...
public class MeetingWebsocketConnectionService {
    ...
    public void registerSessionToMeeting(WebSocketSession session, long meetingId, long userId) {
        ...
        sendEstablishedMessage(session, meetingId, userId);
    }

    private void sendEstablishedMessage(WebSocketSession session, long meetingId, long userId) {
        Long lastProcessedChunkId = audioProgressRepository.findLastProcessedChunkId(meetingId, userId)
                .orElse(null);

        SocketResponse<Long> response = SocketResponse.of(SocketResponseType.CONNECTION_ESTABLISHED, lastProcessedChunkId);

        try {
            session.sendMessage(new TextMessage(objectMapper.writeValueAsString(response)));
        } catch (JsonProcessingException e) {
            log.error("Failed to serialize last processed chunk id to string: {}", lastProcessedChunkId, e);
        } catch (IOException e) {
            log.error("failed to send last processed chunk id to session: {}", session.getId(), e);
        }
    }
}
